...
def pagerank_chunk_text_with_page_tracking(text_by_page, model_name="all-mpnet-base-v2", sim_threshold=0.7, expansion_threshold=0.5):
    """
    ...
    for idx in seed_indices:
        ...
        while i >= 0 and i not in used and cosine_similarity(
            [embeddings[i]], [embeddings[chunk[0]]])[0][0] > expansion_threshold:
            chunk.insert(0, i)
            used.add(i)
            i -= 1
        ...
        while i < len(sentences) and i not in used and cosine_similarity(
            [embeddings[i]], [embeddings[chunk[-1]]])[0][0] > expansion_threshold:
            chunk.append(i)
            used.add(i)
            i += 1
    ...
    for idx, (chunk_text, meta) in enumerate(chunks):
        print(f"[chunker] idx={idx} page={meta.get('page')} snippet={chunk_text[:30]!r}")
